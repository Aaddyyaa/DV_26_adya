...
public:
    StateEstimatorEKF() : Node("state_estimator") {
        ...
        this->declare_parameter("gear_ratio", 4.0);
        ...
        imu_sub_ = this->create_subscription<sensor_msgs::msg::Imu>(
            "/imu/data", rclcpp::SensorDataQoS(), [this](const sensor_msgs::msg::Imu::SharedPtr msg) {
                y_raw_(0) = msg->linear_acceleration.x;
                y_raw_(1) = msg->linear_acceleration.y;
                y_raw_(2) = msg->angular_velocity.z; 
            });
...
private:
    ...
    void jointCallback(const sensor_msgs::msg::JointState::SharedPtr msg) {
        ...
        for (size_t i = 0; i < msg->name.size(); i++) {
            if (msg->name[i] == "wheel_rl_joint" || msg->name[i] == "left_rear_axle") {
                v_rl = (msg->velocity[i] / gear_ratio) * r_wheel; found = true;
            } else if (msg->name[i] == "wheel_rr_joint" || msg->name[i] == "right_rear_axle") {
                v_rr = (msg->velocity[i] / gear_ratio) * r_wheel; found = true;
            }
        }
        
        if (found) {
            latest_vx_meas_ = (v_rl + v_rr) / 2.0;
            has_meas_ = true;
        }
    }
    ...
    void runEKF() {
        ...
        if (has_meas_) {
            // H Matrix maps state [vx, vy, r] to measurement [vx]
            Eigen::Matrix<double, 1, 3> H;
            H << 1.0, 0.0, 0.0;
            
            // The Innovation (Error between IMU guess and Wheel Speed reality)
            double z = latest_vx_meas_;
            double y = z - (H * x_hat_)(0); 
            
            // Innovation Covariance (S)
            double S = (H * P_ * H.transpose())(0, 0) + R_meas_;
