...
if (second_ncc > -1.0 &&
    ...
    std::cout
        ...
        << "best=" << best_ncc
        << " second=" << second_ncc
        << " ratio=" << best_ncc / (second_ncc + 1e-9)
        << " thresh=" << cfg.uniqueness_ratio
        << std::endl;
    ...
    const PipelineConfig& cfg)
{
    ...
    // ── Stage 4 gate (Route A2: stop blocking real cones). ─────────────────────
    //   The NCC>=0.45 phantom guard from Route A turned out far too strict for
    //   these rectified patches: real cones rarely cleared 0.45, so only ~1-2
    //   of 7 detections got valid depth, perception published ~1 cone, the
    //   planner had no cone wall, and the car sat still for minutes.
    //
    //   Ghost prevention now lives where the ghosts are born (Stage 1/2:
    //   tighter NMS + same-class centre dedup, already in cone.cpp). The depth
    //   gate's only job here is to refuse a cone whose geometry doesn't add up.
    //   backproject() already only sets a keypoint valid when its depth Z is
    //   inside [min_depth, max_depth], so a cone with >=1 valid ring already
    //   has a physically plausible distance. Accept it.
    if (valid_count < 1 || sum_w < 1e-9) return;
    ...
    const PipelineConfig&          cfg)
{
    ...
    // [GHOST-STAGE4-DEPTH] valid depth estimates (res.valid==true):
    {
        int valid_depth = 0;
        for (const auto& r : results) if (r.valid) ++valid_depth;
        std::cout << "[GHOST-STAGE4-DEPTH] valid_depth=" << valid_depth
